...
function placeBet(stopAt, bet, currentAt, stake, callback){
    ...
    getHash(function(){
        ...
        user.clientSeed = user.clientSeed;
        $.ajax({
            type: "POST",
            ...
            if(data.outcome >= rangeWin){ // loss
                ...
                if(g_stopAt > g_currentAt){
                    ...
                    if(!cashedOut && betting){
                        if(g_currentAt <= 1.016){
                         
                           console.log("Not cashed out, doing next bet");
                           placeBet( parseFloat(g_stopAt), parseFloat(g_bet), parseFloat(g_currentAt+0.0025), parseFloat( ( g_stake * g_currentAt ) ) );
                           return;
                        }else{
                           console.log("Not cashed out, doing next bet");
