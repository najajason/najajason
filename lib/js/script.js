...
// var qStep = prompt("Please enter Stepsize (Chose a number between 0.1 and 99.8 - think of it as the chance of a dicebet)");
// var qqStep = parseFloat(100 - (100 - odds) - qStep);
    ...
    getHash(function(){
        ...
        $.ajax({
            ...
            if(data.outcome >= rangeWin){ // loss
                ...
                var Xroll = parseFloat(g_stake / g_bet);
                if(g_stopAt > Xroll){
                    user.hash = data.next_hash;
                    if(!cashedOut && betting){
                        if(g_currentAt <= 1.009){
