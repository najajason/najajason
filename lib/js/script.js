...
function placeBet(stopAt, bet, currentAt, stake, callback){
    ...
    getHash(function(){
        ...
        var rangeWin2 = Math.floor(Math.pow(2,32)*(odds));
        var rangeWin = parseInt(Math.floor(Math.pow(2,32)*(odds)) - rangeWin2);
        user.clientSeed = getSeed();
        $.ajax({
            type: "POST",
            ...
            data: JSON.stringify({
                ...
                "payouts": [
                    {from: 0, to: rangeWin, value: 0)},
                    {from: rangeWin, to: Math.pow(2,32), value: ((g_stake*g_currentAt)*100}
                ]
            }),
            dataType: "json",
            ...
            if(data.outcome >= rangeWin){ // loss
                ...
                cell1.innerHTML = parseFloat((g_stake / g_bet)).formatMoney(2,'.',',');
